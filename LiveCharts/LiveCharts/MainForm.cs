...
namespace LiveCharts
{
    public partial class MainForm : Form
    {
        ...
        {
            SectionListBox.DataSource = presenter_.GetAllService();
            SectionListBox.DisplayMember = "Name";
            if (SectionListBox.Items.Count > 0)
            {
                presenter_.ShowVisitsBySection(((Service)SectionListBox.Items[0]).Name);
            }
        }
        public MainForm()
        {
        ...
        {
            Service selectedSection = ((Service)(SectionListBox.SelectedItem));
